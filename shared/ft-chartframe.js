...
function drawFrame() {
    ...
    function frame(p) {
        ...
        var subYOffset = chart.select('.header').node().getBBox().height;
        
        header.append('text')
            .attr('class', 'subtitle')
            .attr('x', margin.left)
            .attr('y', margin.top + subYOffset)
            .text(subtitle)
            .attr('dy',0)
            .call(wrap, width - (margin.left + margin.right), margin.left);

        if(border=='print') { //adds the hat and basline to the print version only
            chart.append('path')
	            .attr('class','hat')
    ...
    frame.styleSheet = function(cssFile){
    	if(!cssFile) return styleSheet;
    	styleSheet = cssFile;
    	return frame;
    }

    frame.plotDimension = function(){
    	return plotDimension;
    }

    frame.border = function(type){
    	if(!type) border;
    	border = type;
    	return frame;
    }

    frame.style = function(s){
    	if(!s) return style;
    	style = s;
    	return frame;
    }
...
function parseDimension(value){
	...
	if( isNaN(value) && ( value.indexOf('mm') > 0) ){
		return mmToPx( value.split('mm')[0] );
	}
	return value;
}

//load a file and append the contet to the parent node as text
function loadStyle(p, location){
	d3.request(location)
	    .get(function(xhr){
